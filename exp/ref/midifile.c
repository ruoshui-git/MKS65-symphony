...
find_tempo()
{
    ...
    {
        if (tempo_history_time[i] <= Mf_currtime)
            old_tempo = tempo_history[i];
        new_tempo = tempo_history[i];
        if (tempo_history_time[i] > revised_time)
            break;
    }
...
metaevent(int type)
{
    ...
    case 0x51: /* Set tempo */
        ...
        if (tempo_history[tempo_history_count] == Mf_currtempo)
            break;
        if (tempo_history_time[tempo_history_count] > Mf_currtime)
            break;
        if (tempo_history_count < MAX_HISTORY - 1)
            tempo_history_count++;
        tempo_history[tempo_history_count] = Mf_currtempo;
...
static void
    chanmessage(status, c1, c2) int status;
int c1, c2;
{
    int chan = status & 0xf;
    ...
    if (c1 > 127) /*mferror("chanmessage: bad c1") ??*/
        return;
    if (c2 > 127)
        c2 = 127;
 ...
 */
void
    mfwrite(format, ntracks, division, fp) int format,
    ntracks, division;
FILE *fp;
{
    int i;
...
void
    mf_write_track_chunk(which_track, fp) int which_track;
FILE *fp;
{
...
void
    mf_write_header_chunk(format, ntracks, division) int format,
    ntracks, division;
{
 ...
 */
int
    mf_write_midi_event(delta_time, type, chan, data, size) unsigned long delta_time;
int chan, type;
unsigned long size;
char *data;
{
 ...
 */
int
    mf_write_meta_event(delta_time, type, data, size) unsigned long delta_time;
...
void
    mf_write_tempo(delta_time, tempo) unsigned long delta_time;
unsigned long tempo;
{
    /* Write tempo */
...
void
    mf_write_seqnum(delta_time, seqnum) unsigned long delta_time;
unsigned seqnum;
{
...
unsigned long
    mf_sec2ticks(secs, division, tempo) int division;
unsigned long tempo;
double secs;
{
 ...
 */
void
    WriteVarLen(value) unsigned long value;
{
 ...
 */
double
    mf_ticks2sec(ticks, division, tempo) int division;
unsigned long tempo;
unsigned long ticks;
{
 ...
 */
void
    write32bit(data) unsigned long data;
{
